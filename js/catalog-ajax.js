...
(function ($) {
    ...
    // Пример результата: ?pa_brand=isover,knauf&pa_insulation-type=basalt&paged=2
    function updateUrl() {
        var params  = new URLSearchParams();
        var grouped = {};

        $sidebar.find('input[type="checkbox"]:checked').each(function () {
            var name = $(this).attr('name').replace('[]', ''); // filter_pa_brand
            if (!grouped[name]) grouped[name] = [];
            grouped[name].push($(this).val());
        });

        $.each(grouped, function (name, values) {
            // Убираем префикс filter_: pa_brand=isover,knauf
